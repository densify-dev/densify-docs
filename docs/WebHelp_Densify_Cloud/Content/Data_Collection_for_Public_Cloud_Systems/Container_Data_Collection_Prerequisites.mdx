...
The following software is required for Kubex container data collection and optimization.

1. Kubex account—Contact Kubex for details of your subscription or sign up for a free trial.
...
When deploying Prometheus and kube-state-metrics using a standard operator, some of the metrics that Kubex needs for analysis may be excluded (i.e. on a deny list). Refer to [Prometheus-Data](https://github.com/densify-dev/container-data-collection/tree/main/docs) for details of the list of metrics that Kubex requires for analysis.
...
After deploying the data forwarder, contact [[email protected]](/cdn-cgi/l/email-protection) to enable your Kubex instance with container optimization.
