...
sidebarTitle: "CloudFormation Template"
---
...
Kubex's CloudFormation template creates a <Tooltip tip="A collection of AWS resources that are managed together as a single unit.">stack</Tooltip> comprised of the AWS resources required by Kubex to collect the Resource Utilization Metrics (via CloudWatch) that are then used for cloud optimization analysis.

If your accounts are setup using an AWS organization/management account structure, in which many accounts are linked to one management account, you can use the CloudFormation template to create, update and delete StackSets across multiple accounts and also create the cloud connection within Kubex for each of the linked accounts.
...
This template creates the IAM role and access policy and assigns the policy to the role using native CloudFormation capabilities. The template then uses a Lambda function to make an API call to Kubex to create the cloud connections required to collect resource utilization data.

If you need to update the IAM role or policy after they have been created, see [Editing the Stackset](#editing-the-stackset) . This may be necessary if the IAM access policy needs additional permissions.

## Supported Regions

You can run the Kubex CloudFormation template in any of the following regions;
...
The Kubex CloudFormation template must be run in a single region even if the target accounts span multiple regions. The template creates an IAM role and permission policy which are both global and are not region-specific.

Please contact [Support@Kubex.ai](mailto:Support@Kubex.ai) if you need to run the template in regions, not listed above.
...
The Lambda function is region-specific but it only runs once to create the cloud connections within Kubex. The StackSet and the CloudWatch logs that result from running the template are also region-specific and are defined within a specific region. You need to log into the specified region to manage your Stack and view its related log files. These log files will be required by the Kubex support team, if troubleshooting any data collection issues.

## Using the CloudFormation Template

If your AWS accounts are setup using an organizational structure, in which many accounts are linked to one master account, you can use the CloudFormation template to create, update and delete stacks across multiple accounts and also create the cloud connection within Kubex for each of the linked accounts. If you are using the organizational account structure, you need an IAM user that has sufficient execute permissions to create the required roles and IAM policies in each linked account.
...
* External ID for IAM Role--This value is similar to a password and should be unique and difficult to guess. Kubex recommends using a password generator to create a random, alphanumeric string (e.g. ae73mcf4ldjpet96) for the external ID.
* Both the URL and user credentials for your Kubex instance--Contact your Kubex Account Manager for details if you do not have this information.
...
2. At the top of the StackSets page, select a supported region. See the list of [Supported Regions](#supported-regions).
...
7. In the next section, Specify template copy and paste the appropriate URL into the Amazon S3 URL field: https://kubex-cf-template.s3.us-east-2.amazonaws.com/Kubex_CloudFormation.yml
   For new Kubex deployments, use the Kubex CloudFormation template:

   https://kubex-cf-template.s3.us-east-2.amazonaws.com/Kubex_CloudFormation.yml

   If you already have a Densify CloudFormation StackSet deployed, continue using the Densify template for StackSet updates. Existing Densify deployments cannot be switched to the Kubex template.

   https://densify-cf-template.s3.us-east-2.amazonaws.com/Densify_CloudFormation.yml
...
* Stack name--Kubex-Data-Collection, as an example.
...
2. Enter the following Kubex instance details:

* Kubex Instance URL--This is your Kubex SaaS instance.
* Kubex User ID--This is your Kubex user name.
* Kubex Password--This is the password for the listed user name.

<Accordion title="Figure: Specify Kubex Instance Details ">
...
Do not set this value greater than 10. Kubex highly recommends batching your accounts into smaller groups for improved performance.

Setting a higher number is faster for AWS operations, but creating the cloud environments and initiating data collection within Kubex is negatively impacted if more than 10 accounts are run concurrently.
...
5. Click Next to proceed with creating the stack.
...
4. You can also log into your Kubex instance and verify that the cloud connections are listed in the Cloud Connection wizard.
...
If your accounts are setup using an organizational structure you can use the CloudFormation template to update stacks across the multiple linked accounts and also create the cloud connection within Kubex for each of the linked accounts.
...
4. You need to remove any deleted accounts from Kubex. See [Deleting a Connection](../Densify_Com/Using_the_Public_Cloud_Connection_Wizard#Deleting) for details.
