...
Kubex collects resource utilization metrics (CloudWatch data) for your AWS services (e.g. EC2, RDS, ECS, etc.), analyzes the AWS data and then makes recommendations to save costs and reduce risks in your AWS environments.

Collecting data via a cross-account IAM Role simplifies the process of connecting to multiple AWS accounts from Kubex since the same role and external ID can be used across your multiple AWS accounts. As accounts are added or removed, you do not need to update the Kubex cloud connection.
...
Kubex also supports connecting to AWS using;
...
<Note>
Though the cloud connection wizard provides the option to use an IAM user and an access key, Kubex recommends using the IAM Role.
</Note>
...
When you create a role for cross-account access, you establish trust from the customer's account that owns the role and the resources (trusting account) to the Kubex account containing the user that will collect data (trusted account). You specify the trusted account number as the Principal in the role's trust policy when you create the role. This allows the Kubex user in the trusted account to assume the role and collect utilization data.
...
4. Enter an Account ID. This is the Kubex account that will assume the role. Enter the following Kubex account ID: 036437403198.
5. Select Require external ID and enter your external ID. This value is similar to a password and should be unique and difficult to guess. Kubex recommends using a password generator to create a random, alphanumeric string (e.g. ae73mcf4ldjpet96) for the external ID.

You will need this external ID later, when creating the cloud connection in Kubex.
...
9. In the Review page, specify the Role name and Role description. The role name can be any string used to identify and describe the role within the AWS account (e.g. KubexCrossAccountRole).
...
12. Copy and save the Role ARN as you will need to paste this string into the Kubex Cloud Connection wizard to create the connection.
13. You can now create an AWS connection through the Kubex Public Cloud Connection wizard. See [Using the Public Cloud Connections Wizard](../Densify_Com/Using_the_Public_Cloud_Connection_Wizard).

You can also use the Kubex API. See [Setting up AWS Connections](/WebHelp_Densify_API_Cloud/Content/API_Guide/Analysis_AWS_Analyze).
...
To simplify setup and maintenance of either an IAM user account or an IAM role for performing the CloudWatch audit, Kubex recommends attaching the AWS-managed “ReadOnlyAccess” policy to the user or role. This policy provides read-only access to your AWS services and resources and supports the requirements of the Kubex CloudWatch audit.
As the Kubex CloudWatch audit continues to evolve and expand, you do not need to update permission policy to include newly added services and features.
...
<Note>
This custom policy must be updated periodically as Kubex standard audit requirements are updated to support additional AWS services and features.
</Note>
...
4. Review the policy and enter a policy name (e.g. KubexMinimumReadAccess) and a description (e.g. Minimum permissions required for Kubex standard audit).

### Enabling Collection of AWS Memory Usage Metrics

Memory metrics are not collected by default and they are not required to complete the Kubex analyses; however, you can manually enable collection of specific memory metrics to improve the quality of your recommendations.
...
<Note>
The CloudWatch agent must be installed and configured on each instance for which you want to obtain memory. Refer to the AWS user documentation for details. See \(https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/Install-CloudWatch-Agent.html)
</Note>



Once the CloudWatch agent is installed and configured, Kubex requires that you use the default "CWAgent" namespace for the collected metrics collected.
...
For Linux instances, the memory metric, “mem\_used\_percent” is collected by default, as specified in the config.json file. However, the metrics “mem\_active” and “mem\_used” need to be added for Kubex analysis.

Use the following example to customize the default CloudWatch agent's configuration, to collect the required parameters and metrics that Kubex requires for memory analyses. The CloudWatch agent configuration file is located in the CWAgent namespace,
...
The memory metric, "% Committed Bytes in Use" is collected by default, as specified in the config.json file. However, the metric “Available MBytes” needs be added to the CloudWatch agent's settings, for Kubex analysis.

Use the following example to customize the default CloudWatch agent's configuration, to collect the required parameters and metrics that Kubex requires for memory analyses. The CloudWatch agent configuration file is located in the CWAgent namespace,

Use the following example to customize the default CloudWatch agent configuration to collect the required parameters and metrics that Kubex requires for memory analyses.
...
Kubex requires the following GPU metrics:
