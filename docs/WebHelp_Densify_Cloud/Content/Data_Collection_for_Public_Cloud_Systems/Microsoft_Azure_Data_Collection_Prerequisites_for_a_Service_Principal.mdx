...
A Service Principal is an identity created within your on-premises Active Directory (AD) and that is allowed to access one or more resources within your AD. You can delegate the minimum required permissions to the service principal so that it can be used to create a connection to Kubex to collect data.
...
The following procedure allows you to use a federated Microsoft Entra ID account to create a cloud connection to Kubex. In this case a Service Principal is required to connect Kubex to your Azure subscriptions.
...
To connect Kubex to your Azure subscriptions, you need the following items to setup data collection:

* [Tenant ID/Directory ID](#Tentant_ID)
* [Service Principal/Application ID](#Secret_Key)
* [Secret Key](#Secret_Key)
* [Subscription ID](#SubscriptionID)--Only required when using the Kubex API.
...
You must have admin/owner privileges for your Azure portal to create the account required for Kubex. Typically, your on-premise Active Directory is federated with the Azure AD. In this case, the admin access is likely provided from an on-premise Active Directory account (via federation) and will possess the required Azure admin privileges.
...
If necessary you can configure a custom role that is more restrictive than the "Reader" role privileges. See [Configuring a Role with Minimum Permissions for Data Collection](#configuring-a-role-with-minimum-permissions-for-data-collection) for details.
...
You now need to create a new application registration. This will be the service principal for Densi. You will need the Application ID and corresponding client secret (key), to complete the Kubex connection.
...
* The Name of the application (e.g. Kubex\_Connection).
...
* Select the Redirect URI (optional) as “Web” and specify a Sign-on URI (e.g. https://Kubex.ai).

The Register button becomes available once you enter valid data.

4. Click Register to create and register the application. This is the service principal that Kubex will use to collect data.
...
5. Copy the Application ID (e.g. 7d16fbf8-1fc3-4e08-b48a-626).
   You need to copy this ID and save it to a location from which you can easily retrieve it. You will need the Application ID to create the Kubex connection.
...
8. In the Add a new client secret pane, enter a Description (e.g. KubexKey) and an expiration period (e.g. 1 year, 2 years or never expires).
9. Click Add to create the key.
10. Copy the secret key Value to a location from which you can easily retrieve it. You will need this key to create the Kubex connection.
...
3. Copy the Subscription ID. You need to copy this ID and save it to a location from which you can easily retrieve it. You will need the Subscription ID to initiate data collection, when using the Kubex API.
...
9. Search for or scroll to locate the service principle (Kubex\_Connection), created above.
10. Click Select.
...
13. Repeat this process for each subscription to be included in Kubex data collection.
...
To simplify setup and maintenance of the role used for performing data collection, Kubex recommends using the "Reader" role. This role provides read-only access to your Azure services and resources and supports the requirements for resource utilization data collection.
As the Kubex continues to evolve and expand, you do not need to update permission policy to include newly added services and features, when using the "Reader" role.

Alternatively, if you must restrict the role with the minimum permissions you can create a custom role with only the required permissions. These custom roles provide an alternative method for granting permissions that are more restrictive than the built-in “Reader” role. These custom roles (JSON) define the minimum permissions required by Kubex to collect resource utilization metrics data, respectively.

The "Kubex Resource Utilization Metrics Reader” custom role grants read-only permissions for collecting data related to VMs, SQL servers and reservations in the subscriptions specified in the assigned scope.

Custom Role: Kubex Resource Utilization Metrics Reader

```json
{
    "properties": {
        "roleName": "KubexResourceUtilizationMetricsReaderCustomRole",
        "description": "This custom role defines the minimum read-only permissions required by Kubex for collecting data related to VMs, SQL servers and reservations associated in the subscriptions specified in the assigned scope",
        "assignableScopes": [],
        "permissions": [
            {
                "actions": [
...
10. Assign the custom role to the Kubex service principal. See [Assigning Access to Subscriptions](#Access_to_Subscriptions).

### Create the Service Principal Through the Azure CLI

If you are comfortable working with the command line, you can create the service principle through the Azure CLI. You can then use the resulting .JSON file when working with the Kubex API. Refer to the Azure website for details: [Create an Azure service principal with the Azure CLI](https://learn.microsoft.com/en-us/cli/azure/create-an-azure-service-principal-azure-cli).

## Creating the Cloud Connection in Kubex
...
When you create the Azure cloud connection for the first time, Kubex discovers all of the subscriptions, associated with the user or service principal. Upon saving the connection it will schedule data collection from each of the discovered and selected subscriptions.
