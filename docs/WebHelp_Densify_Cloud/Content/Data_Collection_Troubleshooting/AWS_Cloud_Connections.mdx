...
sidebarTitle: "Troubleshooting AWS"
...
Solution:Ensure the [AWS Data Collection Prerequisites for an IAM Role](../Data_Collection_for_Public_Cloud_Systems/AWS_Data_Collection_Prerequisites_for_an_IAM_Role__CloudWatch_only_) are setup correctly, Including that the External ID you have entered corresponds to the correct IAM Role.

Once the prerequisites and credentials have been corrected, you can verify the connection on the Public Cloud Connection page.
</Accordion>
<Accordion title="Cause: Systems within the AWS account are not supported">
Solution: If your account only contains services that Kubex does not analyze, then the audit will fail. i.e. DynamoDB, Redshift, Lambda, ElasticCache, etc.

Within Kubex you need to manually delete the Cloud Environments associated with these accounts.
</Accordion>
