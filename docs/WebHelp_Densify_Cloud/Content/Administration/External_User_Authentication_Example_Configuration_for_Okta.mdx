...
You need to register Kubex as a web application in your Okta Authorization Server. and then provide the following information to Densify:
...
4. On the *New Web App Integration* page, select the following settings are click *Save*:
    ...
    * Sign-out redirect URIs:
        ...
        * https://\<yourcompany>.densify.com:443/ (for the Kubex Console)
...
4.	Select *Token Preview* tab and enter the following settings then click *Preview Token*:
    ...
    c.	User – A user that will access Densify
...
You now need to provide the Client ID, Client Secret and Issuer URI, to complete the setup on the Kubex server side. These settings are all configured for you by Densify. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.
