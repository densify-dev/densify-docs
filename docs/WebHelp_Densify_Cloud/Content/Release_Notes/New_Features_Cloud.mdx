...
- [Kubex Automation Controller](#kubex-automation-controller) -- Updates to Kubex's Automation Controller module.
...
<Accordion title="4.4.0 April 29, 2026">
    ...
    - **Oracle Cloud Infrastructure (OCI) Support**
      - Kubex now supports OCI connections from the Cloud Connections page
      - Data collection and analysis are supported for OCI Virtual Machines (VMs)
      - OCI resources are displayed across the UI similar to AWS EC2 and Azure VMs
      - App Owner report and GraphQL API support are available

    - **Container Ephemeral Storage & Optimization Enhancements**
      - Kubex now provides ephemeral storage recommendations in both the UI and REST API
      - Ephemeral storage policies have been introduced

    - **Widget Enhancements**
      - Kubex introduces a GPU Trend widget, providing visibility into GPU usage trends over time
      - Kubex introduces the Prioritizer Insights widget, providing a personalized summary of recommended actions to focus on based on environment and preferences

     - **Policy Enhancements**
      - Policy Viewer layout has been improved for better readability and navigation
      - Kubex now supports configuring samples percentile in Container and Cloud policies 

     - **API Enhancements**
      - Heartbeat API endpoint for Automation Controller has been added 
        `POST /automation/clusters/{clusterName}/heartbeat`

     - **UI Improvements**
      - Kubex now displayes container cost on the Container Overview page
      - Cloud Connections page now supports Edit and Delete functionality
      - Kubex displays a scope limit message in the Container Analysis Details tab (default = 80K manifests)
...
Metrics Viewer  
    - **Automation Tab**--Automation tab has been added to highlight request and limit changes made by the Kubex Automation Controller. An overview of optimized manifests 
...
<Accordion title="3.0.0 April 23, 2025">
  ...
  <Accordion title="Mutating Admission Controller">
    ...
    Refer to [Mutating Admission Controller](https://github.com/densify-dev/helm-charts/tree/master/charts/kubex-automation-controller) for details on deploying and using the Kubex Container Automation solution.
  </Accordion>
...
<Accordion title="4.2.0 - May 15, 2025">
  ...
  <Accordion title="HPA Updates">
    **HPA Scales Based on Multiple Metrics**--With this update Kubex recommendations are not implemented for any containers enabled for HPA that scale based on single or multiple metrics, that are also enabled for automation using the Kubex Mutating Automation Controller (MAC).
...
This section lists new features and updates to the Kubex Automation Controller, which automatically implements Kubex's optimization recommendations for Kubernetes workloads.

A Helm chart bundles all components required for automated container optimization. See [Kubex Automation Controller](https://github.com/densify-dev/helm-charts/tree/master/charts/kubex-automation-controller) for deployment details and configuration options.
