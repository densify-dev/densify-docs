...
<Accordion title="5.2 September 23, 2026">
    The following changes have been made to Kubex in this release:

    - **Updated UI**
      - Designed to make it easier to understand your Kubernetes environment, prioritize optimization opportunities, and measure the impact of the actions you take. 
      - We’re combining the powerful configurability our power users rely on with a simpler, more intuitive experience that makes Kubex easier to navigate and understand for new and occasional users.
      - Ships with a few dashboards (Overview, Waste, Risk, Automation, Know What To Do) and the ability to create your own dashboard. 
      - See the [K8s Dashboards](/docs-kubex/Content/Kubex/K8s_Dashboards) page for details.

</Accordion>

<Accordion title="5.1 September 02, 2026">
    The following changes have been made to Kubex in this release:

    - **Updates to Public Cloud Metadata**
      - Updates to AWS, Azure and OCI for new instance types and costs.

    - **K8s Analysis Policy Updates**
      - Ability for users to change the default policy. Any new clusters added to Kubex will use this default policy.
      - Updates to the thresholds on the policies shipped with Kubex. This is to reflect feedback from how users are setting these in the field.

    - **K8s Analysis using Custom Metrics**
      - Kubex uses CPU and Memory metrics for optimizing Containers. It can now also use custom cpu and/or memory metrics for optimization. 
      - These could be different metrics that are more accurate for optimization. E.g., Queue Depth, Kafka driven metrics.
      - To help set this up, please contact your Kubex team.

</Accordion>

<Accordion title="5.0 August 12, 2026">
    The following changes have been made to Kubex in this release:

    - **New Container Analysis Framework**
      - Kubex v5.0 introduces a new container analysis framework, delivering near real-time analysis with hourly updates and designed to handle very large Kubernetes environments.

    - **Updated Analysis Policy Framework**
      - Enhanced policy engine with flexible policy assignment at the cluster or namespace level, or dynamically based on Kubernetes labels.

    - **Enhanced Container Cost Estimates**
      - Container cost and waste is calculated based on the cost of the node it runs on.
