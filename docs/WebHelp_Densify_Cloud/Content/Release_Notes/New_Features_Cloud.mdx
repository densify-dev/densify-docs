...
This section lists new features and updates to the Kubex Automation Controller, which automatically implements Densify's optimization recommendations for Kubernetes workloads.
...
<Accordion title="1.0.7 - December 11, 2025 (Major)">
  ...
  <Accordion title="Annotation-Based Automation Control">
    ...
    - **Permanent Exclusions**--Permanently exclude specific workloads from automation by setting the annotation value to `infinite`, providing granular control over which resources are managed by Densify.
...
<Accordion title="1.0.2 - October 21, 2025 (Major)">
  ...
  <Accordion title="Automated Pod Eviction and Resizing">
    ...
    - **Automated Pod Scanning**--The controller continuously scans all pods within the configured scope, evaluating them for optimization opportunities based on Densify recommendations.
