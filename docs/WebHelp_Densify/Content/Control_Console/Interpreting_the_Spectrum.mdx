...
The Kubex rebalancing analysis, which is part of the analytics package, will try to load each host equally, and place each host in the yellow, rather than pack the 3 hosts and leave 5 hosts empty. This is a better way to utilize the existing infrastructure.
...
If your CEI is < 0.50 and you are encountering performance issues, then you have not sized/allocated your VMs correctly. Again use the Kubex recommendations to resolve issues in the environment before adding any new VMs. VM users maybe encountering performance issues while available resources are idle.

### Environment has Available Capacity

This scenario is most commonly seen in mature environments. In this example, the CEI is greater than 0.50, but less than 0.75 (0.50\<CEI\<0.75). The environment appears to be full and when Kubex is installed, the Spectrum shows that more VMs can be added. As indicated above, if there are VMs in the red or yellow, adjust their allocations according to the Kubex recommendations before adding additional capacity.
...
Environment is appropriately full. When determining the number of hosts Kubex always rounds up to next whole number. i.e. as soon as you need 6.01 hosts, the CEI will indicate that you need 7 hosts.
...
Review the generated Kubex recommendations and adjust the vCPU and memory allocations. VM placement changes may also be recommended. Your first step is to verify the required size of the VMs and then adjust the allocations based on Densify’s recommendations.
...
These policy settings may have been set incorrectly or the VMs resource allocation may be insufficient. When the VMs are in the red, Kubex generates recommendations to increase the vCPU and memory allocations. If these values are correct then you need to review the policy settings.
