...
Before you can configure the feature in Densify, you must register Kubex with your identity provider. You need to provide information about the application type, login/logout URLs etc. This is a standard procedure across all applications using OpenID for authentication.
...
<Accordion title="Table: Configuring Kubex with the Identity Provider">
  ...
  <td>
  <p>
  This is the callback URL to your <span class="mc-variable Document.Densify variable">
  Densify</span>

   instance where the identity provider redirects the user after they have been authenticated. 
   </p>

  
  <p>
  https://\<my-densify-instance\>:443/
  </p>
  ...
  <td>
  <p>
  A logout URL is a URL in  your <span class="mc-variable Document.Densify variable">
  Densify</span>

   instance that the identity provider can return to after the user has been logged out of the authorization server. 
  </p>

  <ul>
  <li>
  https://\<my-densify-instance\>/ (for the <span class="mc-variable Document.Densify_Console variable">
  Kubex Console</span>
  ...
  <li>
  https://\<my-densify-instance\>:443/admin (for the <span class="mc-variable Document.Analysis_Console variable">
  Analysis Console</span>
  ...
  <li>
  https://\<my-densify-instance\>:443/openIdError
  </li>
  ...
  <li>
  Email – If not available, <span class="mc-variable Document.Company variable">
  Densify</span>

   will use the preferred user name (preferred_username) value of the claim to verify that the user is registered with <span class="mc-variable Document.Company variable">
  Densify</span>

  . <span class="mc-variable Document.Company variable">
  Densify</span>

   assumes this is an email address.
   </li>
...
When this option is enabled, rather than transmit the user details, the identity provider sends a special, one-time-use authentication code that can be exchanged for an access token. This exchange includes the client_id and client_secret in addition to the authentication code.
...
<Accordion title="Figure: Authentication Code Workflow">
  <Frame>
    ![](/images/docs/WebHelp_Densify/Content/Resources/Images/CiRBA_Administration_Guide/03200038_465x271.png)
  </Frame>
</Accordion>
...
2. You now need to provide the following information to Densify. These settings are all configured for you by Densify. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.
...
Once the configuration has been completed, both Kubex and the user are verified by your authentication provider and then permitted access to Densify, upon user login.
...
This feature only applies to the Kubex application and is not applicable to Densify's REST API.
