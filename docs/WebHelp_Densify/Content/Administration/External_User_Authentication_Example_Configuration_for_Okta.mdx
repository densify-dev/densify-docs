---
title: "External User Authentication Example Configuration for Okta"
---
...
3. Complete the following configuration using the specified values:
    ...
    * Logout redirect URIs:
        ...
        * https://\<name>.densify.com:443/ (for the Kubex Console)
...
5. Click on the Access Policies tab > Add New Access Policy. Enter the following values:
    ...
    * Name--A name for the access policy (example: Kubex Access Policy);
    * Description--A description of the access Policy (example: Kubex Access Policy);
...
You now need to provide the Client ID, Client Secret and Issuer value, to complete the setup on the Kubex server side. These settings are all configured for you by Densify. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.
