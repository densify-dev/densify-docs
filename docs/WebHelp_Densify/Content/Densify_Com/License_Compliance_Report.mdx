...
The License Compliance Report allows you to monitor your Kubex license compliance and then request additional licenses via an expansion quote, when necessary. Since Kubex license compliance is based on constantly changing numbers and types of instances in your various environments this report shows your license usage in an easy-to-understand chart format.
...
Densify uses a factor of 1:1 (one Kubex license for each cloud instance.
...
* Has been included in a Kubex analysis;
...
Daily counts of licensed instances, by type, are stored in the Kubex database to support historical reporting of license usage.
...
* Kubex will analyze instances that are powered off if they are in-scope and they meet the policy requirements (e.g. have 7 days of utilization data available in the last 30 days) so in this case the number of analyzed instances may be larger than the license count.
...
For new customers starting in Kubex v2.9, the licensing model will be based on the method outlined above.

Customers that have ASGs licensed prior to Kubex v2.9 have been using a different licensing model. In these cases, ASGs use a formula of 1 ASG = 4 licenses.

After the upgrade to Kubex v2.9 you will not automatically be switched to the “average” method, outlined above. Contact [support@kubex.ai](mailto:support@kubex.ai) to change to this new method, if desired.
...
* Based on container manifests with 2 manifests using 1 Kubex license i.e. weighting factor for a container is then 0.5.
* Based on the number of pods, with 5 pods using 1 Kubex license, i.e. weighting factor for a container is then 0.2.
...
Both nodes and node cores will never be non-zero at the same time. In fact, only one of manifest/container/node/nodecore/pods will be set (i.e. have a non-zero value). Based on how each customer is using Kubex for their container environments, the Kubex support team configures the license based on the terms of the licensing agreement.

### Determining the License Count for ECS

AWS ECS services are also licensed. ECS analysis results are not reported in the Kubex or Kubex UIs, but in a custom report. Contact your account manager for details.
...
License compliance is defined as whether or not the customer is within the daily count of purchased Kubex licenses allotted to their instance. When determining license compliance Kubex uses the following:
...
1. Log into your Kubex Console and navigate to My Reports > License Compliance.
2. Select the License Compliance report.
3. The License Compliance Daily tab is selected by default. You will see your current compliance data.
4. Review the header to see the status. If you have exceeded your license count and are not compliant. Contact [support@kubex.ai](mailto:support@kubex.ai) for details on expanding your license.
5. Click the License Compliance Monthly tab and select a historical report to review.

<Accordion title="Densify Compliance Report - Compliant">
<Frame>![](/images/docs/WebHelp_Densify/Content/Resources/Images/Cirba_SaaS/03000185.png)</Frame>
</Accordion>

<Accordion title="Densify Compliance Report - Non-Compliant">
<Frame>![](/images/docs/WebHelp_Densify/Content/Resources/Images/Cirba_SaaS/03000186.png)</Frame>
</Accordion>
