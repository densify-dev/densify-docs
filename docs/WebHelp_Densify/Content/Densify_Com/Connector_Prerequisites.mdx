...
The prerequisites to run the Kubex Connector are provided in this topic.
...
Prior to installing or upgrading the Kubex Connector, ensure that the Kubex Connector version is compatible with your Kubex application version. Refer to [Densify Connector Compatibility](./Action_Automation_Appendix) for details.

<Accordion title="Software and Hardware Requirements">
This section provides the prerequisites to install and run the Kubex Connector.
...
This section details the minimum hardware requirements assuming a dedicated machine.Contact [support@kubex.ai](mailto:support@kubex.ai) for the specific requirements for the Mid-range Connector.
...
The Kubex Connector, once installed and configured on or near your vCenter Server, collects data from your vCenters. The connector bundles the collected data and then uploads it to your Kubex instance, where it is analyzed.

The connector uses port 80 to communicate with your configured vCenters and with your Kubex instance. Alternatively, you can use port 443, if you are using HTTPS for security. This is the most common use case.
...
Visit the Kubex website, for additional security considerations:
...
The connector then initiates a connection to your Kubex instance to upload the collected data for analysis. Kubex then detects the data has been sent and loads the data to its local database and begins the analysis automatically. Reports will be available in the Kubex Console, within a few hours.

### Internet Connectivity

The Kubex Connector must have 24 hour by 7 days/week connectivity to the internet.

### vCenter Connectivity

The Kubex Connector must have continuous access to the vCenter Servers/vROps, for daily audits.

### Time Zone Settings

Your Kubex Connector must be in the same time zone as your vCenter Server. If your vCenter Servers are in different time zones, then configure a Kubex Connector for each vCenter. You do not have the ability to offset the hours in the data collection audit template to compensate for different time zones.

### Licensing and Number of Systems

Densify trials are limited to 2000 systems.Contact [support@kubex.ai](mailto:support@kubex.ai) to increase your limit if required.
</Accordion>

<Accordion title="Densify Credentials">
[support@kubex.ai](mailto:support@kubex.ai) will provide your Kubex credentials. These credentials are used to log into your Kubex instance. You can download the connector,
</Accordion>

<Accordion title="VMware Configuration">
The Kubex Connector uses the VMware common API interface to audit each vCenter Server and ESX servers. The following prerequisites must be addressed before you can connect and collect the data required for analyses.
...
Prior to installing or upgrading the PowerCLI version on your Kubex Connector, please check [Densify Connector Compatibility](./Action_Automation_Appendix). Running a PowerCLI version that is incompatible with the Kubex Connector may result in Action Automation failing to execute.

In general you want the latest version of the Kubex Connector that meets your requirements.

See [vCenter and PowerCLI Version Compatibility](./Action_Automation_Appendix_B) for a matrix of which PowerCLI versions are supported with each version of vCenter and with the Kubex Connector.
...
Contact [support@kubex.ai](mailto:support@kubex.ai) for details on creating or configuring a vCenter account and to limit the connector to specific clusters.
...
The workload statistical settings must be configured in vCenter Server to ensure that the required metrics are collected during an audit and that sufficient data coverage is maintained for Kubex analyses.

The workload statistical settings must be configured in vCenter Server and optionally vROps to ensure that memory workloads are collected and that sufficient data is collected for Kubex analyses.
...
By default, for vCenter and vROps the statistical level is set to the minimum (Level 1). If you are not using vROps and collecting data from only vCenter then Kubex recommends updating the Statistics Level to 2, if you want to collect active memory and the disk and network I/O metrics required for Kubex analyses. Collecting active memory is required for VM bump-up recommendations.
...
The Kubex Connector supports token-based authentication when connecting your vCenter Server to the Kubex server instance. Authentication is performed through the Kubex API and does not need to be configured through the Connector.
...
Token-based authentication, for vRealize Operations Manager, is supported by the Kubex Connector. You can enable the option when creating the connection.

For ease of use, if basic authentication is enabled on the vROps server (via the API), then you can use either basic authentication or token-based when you create the Kubex connection, by enabling the setting, "Enable vROps token-based authentication ".
...
If basic authentication is disabled on the vROps server (by default in vROps v8.1 or later), then you must use the token-based authentication when your create the Kubex connection, by using the UI setting.
