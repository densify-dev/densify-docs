...
Filtering is done at the cluster level since most reports provide results at his level. Additionally, you can filter based on cluster/infrastructure group capabilities that have been set via the Capability Definitions These settings are all configured for you by Densify. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.

A default view, "Total Estate" is provided so that you can access Kubex reports and features immediately without having to create a view or filter. The default view breakdown is by Environment. This view can be copied and modified to create custom views but cannot be deleted.
...
When filtering on cluster attributes, the text box under Filter Criteria shows all possible cluster names. If you enter a value, rather than selecting an option from the drop down options, Kubex validates that the value corresponds to at least one cluster.
