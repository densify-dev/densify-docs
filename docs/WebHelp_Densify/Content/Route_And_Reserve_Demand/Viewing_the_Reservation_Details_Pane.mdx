...
When routed to an internal hosting venue, Kubex selects the least full host and datastores, even if policy limits are violated or capacity is in the negative. Note that the host and sensor placement answer is provided only after the environment or infrastructure group is refreshed.
