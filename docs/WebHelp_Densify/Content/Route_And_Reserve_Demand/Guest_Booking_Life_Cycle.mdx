...
* If the guests do not belong to a cloud environment, the booking request is updated with a state of Committed (bypassing the Pending Commit state as there is no environment to refresh). When the corresponding system comes online, you need to mark the booking as Completed. Contact [support@kubex.ai](mailto:support@kubex.ai). If you have access to the API, use the Kubex API, specifically the `PUT /booking/<id>` request with the `status` update.
