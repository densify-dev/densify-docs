...
### Kubex Container Data Collection and Kubernetes Labels

Though Kubex's container data collection will work without the Kubernetes object labels as kube-state-metrics labels, you may want to enable the kube-state-metrics labels for the following use cases:
...
In addition to node labels, Kubex attempts to collect the following data, which can be further used as sort/filter criteria and to generate custom reports:
...
<ol start="1">
  ...
  ```
  <li>
    Optionally, you can specify only the Kubernetes object labels that you need. Contact Support@kubex.ai for details.
  </li>
</ol>
...
Kubex's container data collection supports kube-state-metrics, version 1.5 or higher. If your monitoring stack is running an older version, some metrics listed on Kubex's Github page are absent. In this case, Kubex collects the older metrics. The table below summarizes the older versions of the metrics that have been deprecated, and their replacements.
...
After deploying the data forwarder, contact Support@Kubex.ai to enable your Kubex instance with container optimization.

## Using an Observability Platform

When using an observability platform, data is collected from multiple clusters and/or other sources. The incoming data must be identifiable by a unique set of labels (name:value) for each Kubernetes/OpenShift cluster, from which Kubex is collecting data.
...
<Note>
#### **Note** <br/>
For cAdvisor and node exporter Kubex needs the data from all nodes in the cluster but ONLY for the cluster nodes and not from other instances, like other nodes, VMs or cloud instances that do not belong to the clusters, that are being monitored.  
</Note>
...
<ul>
  ...
  <li>Uploading the data to Kubex.</li>
</ul>
...
If you are collecting data from an external observability platform or uploading the data to Kubex and encounter failure and the Data Forwarder logs indicate the following: <br/>
...
When uploading data to Kubex, this issue may result in failure to upload any data due to authentication failure. Please verify that the username and (encrypted) password are correct, and then contact Support@Kubex.ai for assistance.
If the Data Forwarder logs include text similar to:
```
{"level":"fatal","pkg":"default","error":"HTTP status code: 400, Message: message: Unauthorized,status: 400",...,"message":"failed to initialize Kubex client"}
```
...
Kubex collects 3 memory metric types from cAdvisor: 
