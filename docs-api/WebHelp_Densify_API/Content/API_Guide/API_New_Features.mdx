...
Updated Online Documentation--The online help for Virtualization Optimizer has been updated to remove all cloud and container references. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.
</Accordion>

<Accordion title="Densify 14.8 - August 23, 2023">
<Accordion title="Basic Authentication has Been Deprecated">
Basic authentication for accessing the Kubex API has been deprecated in this release. Only token-based authentication is supported, for enhanced security. See [Authorize](./Authorize) for details.

To ease the transition of any integrations, basic authentication functionality can temporarily be extended to September 30th, 2023. Contact [support@kubex.ai](mailto:support@kubex.ai) for details.
</Accordion>
...
* Kubex 2022: \<customer\_name>.densify.com/densify
* Analysis Console:
...
When accessing the Kubex user interface a malicious actor cannot perform a brute force attack because the user account is locked after three failed login attempts for a predefined timeout period.
...
This is already the behaviour when accessing the Impact Analysis and Recommendation Report through the Kubex Console.
...
See [Working with Auto Scaling Groups](/docs/WebHelp_Densify/Content/Densify_Com/Working_with_ASGs#ECS_and_EKS_clusters) in the Kubex Online Help.
</Accordion>

<Accordion title="Effort Estimate in the Subscription API">
When using the standard API, Kubex does not return an effortEstimate for systems that are ‘Not Analyzed’; however, when using the Subscription API Kubex returns an effortEstimate of "Impossible" for systems that are ‘Not Analyzed’.
